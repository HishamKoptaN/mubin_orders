...
class StudentDatabase {
  ...
  Future<Database> _init() async {
    return await openDatabase(join(await getDatabasesPath(), 'mavystudent.db'),
        onCreate: (db, version) async {
      // First time the database is created
      await db.execute(
          "CREATE TABLE $table (id INTEGER PRIMARY KEY, name TEXT, image TEXT, course TEXT)");
      await db.execute(
  ...
  void updateDarkmode(bool isDarkMode) async {
    await (await database).update("settings", {"value": isDarkMode ? "1" : "0"},
        where: "name = ?", whereArgs: ["dark_mode"]);
  }

  /// Get dark mode setting
  Future<bool> getDarkMode() async {
    final List<Map<String, Object?>> result = await (await database)
        .query("settings", where: "name = ?", whereArgs: ["dark_mode"]);
